...
export class SearchBarComponent implements OnInit {
    ...
    ngOnInit() {
        this.radioButton.valueChanges.subscribe((selectedOption) => {
        ...
        this.queryField.valueChanges
            ...
            .subscribe((queryField) => {
                if (this.selectedOption === 'artists') {
                    this.spotify.searchArtists(queryField).subscribe((response) => {
                        this.results = response.artists.items;
                        // console.log(this.results)
                    });
                } else {
                    this.spotify.searchTracks(queryField).subscribe((response) => {
