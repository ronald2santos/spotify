...
export class AudioFeaturesChartComponent implements OnChanges {
  ...
  getSingleTrackAudioFeatures(id: string) {
    this.spotify.getTracksAudioFeatures([id])
      .pipe(
        map(
          (data) => {
            ...
            return [track.acousticness, track.danceability, track.energy, track.instrumentalness, track.liveness, track.speechiness, track.valence];
          }
        )
      )
      .subscribe(
  ...
  getArtistTopTracksAudioFeatures(id: string) {
    this.spotify.getArtistsTopTracks(id)
      .pipe(
        map((list) => {
      ...
      .subscribe(
        (trackIDs) => {
          this.spotify.getTracksAudioFeatures(trackIDs)
            .pipe(
              map(
                (data) => {
  ...
  calculateChartData(data) {
    ...
    const avgAcousticness = ((data.audio_features.map((ft) => {
      return ft.acousticness;
    }).reduce((a, c) => a + c)) / nTracks).toFixed(3);

    const avgDanceability = ((data.audio_features.map((ft) => {
      return ft.danceability;
    }).reduce((a, c) => a + c)) / nTracks).toFixed(3);

    const avgEnergy = ((data.audio_features.map((ft) => {
      return ft.energy;
    }).reduce((a, c) => a + c)) / nTracks).toFixed(3);

    const avgInstrumentalness = ((data.audio_features.map((ft) => {
      return ft.instrumentalness;
    }).reduce((a, c) => a + c)) / nTracks).toFixed(3);

    const avgLiveness = ((data.audio_features.map((ft) => {
      return ft.liveness;
    }).reduce((a, c) => a + c)) / nTracks).toFixed(3);

    const avgSpeechiness = ((data.audio_features.map((ft) => {
      return ft.speechiness;
    }).reduce((a, c) => a + c)) / nTracks).toFixed(3);

    const avgValence = ((data.audio_features.map((ft) => {
      return ft.valence;
    }).reduce((a, c) => a + c)) / nTracks).toFixed(3);
    ...
    const avgDuration = ((data.audio_features.map((ft) => {
